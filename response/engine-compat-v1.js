(function (root) {
  ...
  const legacyInput = c => c?.model==='compass-v1-legacy' && typeof c.angle==='number' && Number.isFinite(c.angle) && c.angle>=0 && c.angle<360;
  function directionValue(compass){if(!legacyInput(compass)||!compass?.defensible||number(compass.angle)===null)return null;
  ...
  function calculate(state={},summary=null,compass=null,today=iso(new Date())){
    ...
    return {today,dates,components,availableWeight:weight,carryover,D,response:value,displayResponse:value===null?null:Math.round(value),completeDays,direction:legacyInput(compass)&&compass?.defensible?compass.direction:null,directionModel:'compass-v1-legacy',compatibilityError:legacyInput(compass)?null:'legacy-calculation-unavailable'};
  }
  root.MotionCResponse=Object.freeze({directionModel:'compass-v1-legacy',anchors,score,calculate,directionValue,response,previousDays});
