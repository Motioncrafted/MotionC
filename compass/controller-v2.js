(()=>{'use strict';
 ...
 function renderHistory(p){
  ...
  for(const r of rows){const e=document.createElement('details');e.className='history-reading';e.dataset.date=r.asOf;const s=document.createElement('summary');s.textContent=dateLabel(r.asOf);const b=document.createElement('strong');b.textContent=r.angle.toFixed(1)+'°';s.append(b);e.append(s);const p=document.createElement('p');p.textContent='Saved '+new Date(r.savedAt).toLocaleString()+'. '+Math.round(r.coverage*100)+'% recorded coverage.';e.append(p);const technical=document.createElement('details');technical.className='saved-technical';const summary=document.createElement('summary');summary.textContent='Technical details';technical.append(summary);for(const d of r.drivers){const p=document.createElement('p');p.textContent=(d.key==='bodyTrend'?'Body Trend':d.key[0].toUpperCase()+d.key.slice(1))+': '+contribution(d);technical.append(p);}e.append(technical);box.append(e);}
 }
