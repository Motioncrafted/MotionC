(() => {
  ...
  menus.forEach(id=>{const menu=document.getElementById(id);if(!menu)return;menu.classList.add('has-checkin-access');if(['walkingPreferencesMenu','enginePreferencesMenu'].includes(id)){const home=document.createComment('preferences menu home');menu.before(home);const mobile=matchMedia('(max-width:900px)'),place=()=>{if(mobile.matches)document.body.append(menu);else home.after(menu);};place();mobile.addEventListener('change',place);}const group=document.createElement('section');group.className='my-checkin-menu';group.setAttribute('aria-label','My Check-In');const title=document.createElement('h3');title.textContent='My Check-In';group.append(title);
    ...
    const support=document.createElement('section');support.className='preferences-support';support.setAttribute('aria-label','Help & Support');
    const heading=document.createElement('h3');heading.textContent='Help & Support';
    const prompt=document.createElement('p');prompt.textContent='Need help with MotionC?';
    const email=document.createElement('a');email.href='mailto:support@motionc.me';email.textContent='support@motionc.me';
    support.append(heading,prompt,email);menu.append(support);
    const fitMenu=()=>{if(menu.hidden)return;menu.style.setProperty('--preferences-top',Math.max(0,menu.getBoundingClientRect().top)+'px');};
    new MutationObserver(fitMenu).observe(menu,{attributes:true,attributeFilter:['hidden']});
    window.addEventListener('resize',fitMenu);window.visualViewport?.addEventListener('resize',fitMenu);
  });
